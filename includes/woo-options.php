...
class ShipperAdvancedOptionsMetabox
{
    ...
    public function add_meta_boxes()
    {
        foreach ($this->screens as $s) {
            add_meta_box(
                "ShipperAdvancedOptions",
                __("Shipper Advanced Options", "shipper"),
    ...
    public function field_generator($post)
    {
        ...
        if ($order) {
            $exportUrl = get_post_meta($post->ID, 'shipper_order_export_url', true);
                ...
                <script>
                    ( function() {
                        ...
                        jQuery('#submit_form_data').click((e) => {
                            ...
                            const form = $('#ShipperAdvancedOptions').find('input:not([type="hidden"]), select, textarea');
                            ...
                            var baseUrl = `<?php echo get_option("shipper_options")[
                                "shipper_base_url"
                            ];?>`;
                            var settings = {
                                'url': baseUrl,
                            ...
                            if (baseUrl) {
                                $.ajax(settings).done(function (response) {
                                    ...
                                    $('#shipper_export_btn').attr('disabled', false);
                                    $('#shipper_export_btn').attr('href', JSON.parse(response)?.lien);
                                    document.cookie = `shipper_order_export_url_${<?php echo $post->ID; ?>}=${JSON.parse(response)?.lien}`;
                                    <?php
                                        $exportUrl = $_COOKIE['shipper_order_export_url_' . $post->ID];
                                        if ( ! add_post_meta( $post->ID, 'shipper_order_export_url', $exportUrl, true ) ) { 
                                            update_post_meta ( $post->ID, 'shipper_order_export_url', $exportUrl );
                                        }
                                    ?>
                                }).fail(function() {
...
if (class_exists("ShipperAdvancedOptionsMetabox")) {
    new ShipperAdvancedOptionsMetabox;
}